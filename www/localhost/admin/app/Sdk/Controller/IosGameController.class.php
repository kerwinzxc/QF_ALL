...
//	        }
    ...
    public function set_status() {
        ...
        if (2 == $status) {
            ...
            /*ios取包地址为c_game_info中yiosurl字段 start*/
            $_down_url = M("game_info")->where(array('app_id' => $id))->getField('yiosurl');
            if (4 == $g_data['classify']){
                if (empty($_down_url)) {
                    $this->error("请上传游戏母包");
                }
            }elseif(401 == $g_data['classify']){
                if (empty($_down_url)) {
                    $this->error("请在ios游戏管理中填写下载链接");
                }
            }
            /*ios取包地址为c_game_info中yiosurl字段 end*/
            $data['run_time'] = time();
        }
        $data['status'] = $status;
        $rs = $this->game_model->where("id = %d", $id)->save($data);
