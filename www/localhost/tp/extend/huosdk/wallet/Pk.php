...
class Pk {
        ...
        try {
            ...
            if ($_rs) {
                ...
                if ($_gm_info) {
                    ...
                    $this->_error('无清风币,金额错误:'.json_encode($paydata).'error');
                }
            }

            return true;
        } catch (\Exception $e) {
            $this->_error('清风币插入失败 错误信息:'.$e->getMessage().'error');
    ...
    public function rebate(array $paydata, $appid = 0) {
        ...
        try {
            ...
            $this->_error('清风币充值 错误信息:'.$e->getMessage(), 'error');
     ...
     *
     * orderinfo.money          是    FLOAT    玩家充值清风币金额;建议传入整数,可以保留两位小数
     * orderinfo.real_money     是    FLOAT    玩家实际支付金额
     * orderinfo.gamemoney      是    FLOAT    玩家可获得的清风币
