 ...
 * @return mixed
 */
function aliyunSend($mobile, $smstemp = '', $product = '') {
    Loader::import('aliyun.aliyun-php-sdk-core.Config');
    Loader::import('aliyun.Dysmsapi.Request.V20170525.SendSmsRequest');
    Loader::import('aliyun.Dysmsapi.Request.V20170525.QuerySendDetailsRequest');
    $aliyun = dirname(dirname(APP_PATH))."/conf/sms/aliyun.php";
    //获取阿里大鱼配置信息
    if (file_exists($aliyun)) {
        $_config = include $aliyun;
    } else {
        $_config = array();
    }
    if (empty($_config)) {
