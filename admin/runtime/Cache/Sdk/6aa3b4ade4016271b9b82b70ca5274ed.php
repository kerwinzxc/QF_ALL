...
<div class="wrap js-check-wrap" >
    <ul class="nav nav-tabs" >
        <li class="active" ><a href="<?php echo U('Sdk/Game/index');?>" >游戏列表</a ></li >
        <li ><a href="<?php echo U('Game/delindex');?>" >删除列表</a ></li >
        <!--<li><a href="<?php echo U('Game/add');?>">添加游戏</a></li>-->
    </ul >

    <form class="well form-search" method="get" action="<?php echo U('Game/index');?>" >
                    ...
                    <select class="select_2" name="status" id="selected_id" >
                        ...
                        <?php if(1 == C('G_GAME_FROM')): ?>&nbsp;&nbsp;
                            ...
                            <select class="select_2 js-example-basic-single" name="classify" id="selected_id" >
                                ...
                                <?php if(is_array($gamefrom)): foreach($gamefrom as $k=>$vo): $g_select=($k==$formget['classify'] ?"selected":""); ?>
                                    <option value="<?php echo ($k); ?>" <?php echo ($g_select); ?> ><?php echo ($vo); ?></option ><?php endforeach; endif; ?>
                            </select ><?php endif; ?>
    ...
    <form class="js-ajax-form" action="" method="post" >
        <table class="table table-hover table-bordered table-list" >
            ...
            <tr >
                ...
                <?php if(1 == C('G_GAME_FROM')): ?><th width=50 >游戏来源</th ><?php endif; ?>
                <th width=50 >状态</th >
            ...
            <?php if(is_array($items)): foreach($items as $key=>$vo): ?><tr >
                    ...
                    <?php if(1 == C('G_GAME_FROM')): ?><td > <?php echo ($gamefrom[$vo[classify]]); ?></td ><?php endif; ?>

                    <td >
                        <?php echo ($gamestatues[$vo[status]]); ?>
                    </td >
                    <td >
                        <?php if($vo['status'] == 1 OR $vo['status'] == 3): ?><a href="<?php echo U('Game/set_status',array('id'=>$vo['id'],'status'=>2));?>"
                               class="js-ajax-dialog-btn" data-msg="确定上线游戏？" >上线游戏</a >
                            <?php else: ?>
                            <?php echo (date('Y-m-d H:i:s',$vo["run_time"])); ?><br />
                            <a href="<?php echo U('Game/set_status',array('id'=>$vo['id'],'status'=>3));?>"
                    ...
                    <td style="word-wrap:break-word;word-break:break-all; " >
                        <?php if(empty($vo['packageurl'])): ?>暂无母包(<?php echo ($vo['initial']); ?>)
                            ...
                            <a href="javascript:;"
                               class=" link_copy_btn "
                               data-clipboard-text="<?php echo DOWNSITE;?>/<?php echo ($vo["packageurl"]); ?>"
                            >复制</a >
                    ...
                    <td >
                        ...
                        |
                        <a href="<?php echo U('Sdk/Game/edit',array('id'=>$vo['id']));?>" >编辑 </a >
                        <?php if (C('G_OA_EN')) { ?>    |
                        <a href="javascript:;" class="checkOaGame" ahref="<?php echo U('Sdk/Game/checkOaGame',array('id'=>$vo['id']));?>" >同步到oa </a >
                        <?php } ?>
                        <?php if($vo['status'] != 0): ?>|
                            <a href="<?php echo U('Game/delGame',array('id'=>$vo['id']));?>" class="js-ajax-delete" > 删除</a ><?php endif; ?>
                    </td >
...
<div class="pop_up_form_div container" id='add_game' style="display:none;" >
    ...
    <div class="row" >
        ...
        <div class="col-md-8" >
            <input type="text" name="add_game_name"  style="width:210px"/>
        </div >
    </div >
    <?php if(1 == C('G_GAME_FROM')): ?><div class="row" >
...
<script >
    ...
    $("#agent_submit_btn").click(function () {
        ...
        var data     = {"name": name, "classify": classify , "cp_id":cp_id};
        $.post(url, data, function (res) {
            if (res.error == "0") {
                yxalert(res.msg);
                reload_delay();
